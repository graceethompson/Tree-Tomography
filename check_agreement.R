...
dat <- read_csv("Tree_ID_info.csv", show_col_types=FALSE) %>% rename_all(tolower)
ert <- read_csv("ERT_application_results.csv", show_col_types=FALSE) %>% rename_all(tolower)
dat <- dat %>%
...
hem_val_ert <- read_csv("Hemlock ERT/validation_summary.csv", show_col_types=FALSE)
