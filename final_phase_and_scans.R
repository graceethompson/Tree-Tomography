...
hem_ert <- read_csv("Hemlock ERT/validation_summary.csv", show_col_types = FALSE)
hem_sot <- read_csv("Hemlock ERT/SOT_results.csv", show_col_types = FALSE)
...
ggsave("pca_biplot.pdf", p_biplot, width = 9, height = 7)
cat("Saved: pca_biplot.pdf\n")
...
ggsave("final_phase_diagram.pdf", p_final, width = 9, height = 8)
cat("\nSaved: final_phase_diagram.pdf\n")
...
hem_ert_raw <- read_csv("Hemlock ERT/ERT_results_2026-03-05.csv", show_col_types = FALSE)
...
# Load core moisture
mc <- read_excel("Hemlock ERT/MC_Tomo_paper_Jon.xlsx", sheet = 1)
...
make_scans_by_height(multi_height_trees, tree_order$tree_id,
                      metric_rows_add,
                      "Hemlock ERT/fig_scans_by_height_with_pc1.pdf",
...
make_scans_by_height(multi_height_trees, tree_order$tree_id,
                      metric_rows_replace,
                      "Hemlock ERT/fig_scans_by_height_pc1_only.pdf",
                      "ERT Scans by Height (sorted by core moisture)")
...
ggsave("Hemlock ERT/fig_best_predictor_pc1.pdf", fig_bp,
       width = 7, height = 6, device = cairo_pdf)
cat("Saved: Hemlock ERT/fig_best_predictor_pc1.pdf\n")
...
ggsave("quadrant_distribution.pdf", fig_combined, width = 12, height = 14)
cat("Saved: quadrant_distribution.pdf\n")
